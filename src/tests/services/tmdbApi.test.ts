...
describe('TMDB Service', () => {
  ...
  it('should fetch popular movies', async () => {
    vi.mocked(tmdbApi.default.getPopularMovies).mockResolvedValue(
      mockTMDBResponse
    );
  ...
  it('should fetch now playing movies', async () => {
    vi.mocked(tmdbApi.default.getNowPlayingMovies).mockResolvedValue(
      mockTMDBResponse
    );
  ...
  it('should fetch movies by genre', async () => {
    const genreId = 28;
    vi.mocked(tmdbApi.default.getMoviesByGenre).mockResolvedValue(
      mockTMDBResponse
    );
  ...
  it('should fetch movie videos', async () => {
    const movieId = 123;
    vi.mocked(tmdbApi.default.getMovieVideos).mockResolvedValue(
      mockVideosResponse
    );
  ...
  it('should get trailer key from videos', () => {
    ...
    expect(tmdbApi.default.getTrailerKey).toHaveBeenCalledWith(
      mockVideosResponse
    );
