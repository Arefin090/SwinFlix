<template>
  <div class="relative min-h-screen bg-black">
        ...
        <div class="flex justify-between items-center px-8 py-6">
          ...
          <div class="relative">
            <button 
              ...
              class="flex items-center space-x-2 bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 px-4 py-2 rounded-lg transition-all duration-300"
            >
        ...
        <div class="px-8 max-w-3xl">
          ...
          <div class="flex space-x-4 mb-8">
            <button 
              ...
              class="bg-white text-black font-bold py-4 px-8 rounded flex items-center space-x-2 hover:bg-gray-200 transition-all duration-300 transform hover:scale-105"
            >
            ...
            <button 
              ...
              class="bg-gray-600/80 text-white font-bold py-4 px-8 rounded backdrop-blur-sm hover:bg-gray-500/80 transition-all duration-300 transform hover:scale-105"
            >
              More Info
            </button>
          </div>
    ...
    <div class="relative bg-black">
      ...
      <div class="px-8 py-12">
        <div class="max-w-2xl mx-auto relative">
          <input
            ...
            class="w-full p-4 pl-12 bg-gray-800 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all"
          />
      ...
      <!-- Content Sections -->
      <div v-else class="px-8 space-y-16">
        <!-- Genre Selection -->
        <section>
          ...
          <div class="flex flex-wrap gap-4">
            <button
              ...
              class="bg-gray-800 hover:bg-red-600 text-white px-6 py-3 rounded-lg transition-all duration-300 transform hover:scale-105 font-medium"
            >
              {{ genre }}
            </button>
          </div>
        ...
        <section v-if="genreMovies && genreMovies.length">
          <h2 class="text-3xl font-bold text-white mb-8">{{ selectedGenre }} Movies</h2>
        ...
        <!-- Now Playing -->
        <section>
          <h2 class="text-3xl font-bold text-white mb-8">Now Playing</h2>
          <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            <MovieBooking
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click="closeModal">
      <div class="bg-gray-900 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" @click.stop>
        <div class="relative">
          <!-- Close Button -->
          <button 
            @click="closeModal"
            class="absolute top-6 right-6 z-20 bg-gray-800/90 hover:bg-gray-700 text-white rounded-full p-3 transition-all duration-200 shadow-lg"
          >
          ...
          <!-- Movie Info -->
          <div v-if="selectedMovie" class="p-8 space-y-6">
            <div class="flex items-start gap-6">
              <div class="flex-1 min-w-0">
                <h3 class="text-4xl font-bold text-white mb-4 leading-tight">{{ selectedMovie.title }}</h3>
                <div class="flex items-center space-x-4 text-sm mb-6">
                  ...
                  <span class="border border-gray-400 text-gray-400 px-3 py-1 rounded-full text-xs font-medium">HD</span>
                </div>
                <p class="text-gray-300 leading-relaxed text-lg">{{ selectedMovie.overview }}</p>
              </div>
              <img 
                :src="`https://image.tmdb.org/t/p/w300${selectedMovie.poster_path}`" 
                :alt="selectedMovie.title"
                class="w-40 h-60 object-cover rounded-xl shadow-lg flex-shrink-0"
              >
            </div>
            ...
            <div class="flex flex-wrap gap-4 pt-6 border-t border-gray-700">
              <button 
                ...
                class="bg-white text-black font-bold py-4 px-8 rounded-lg flex items-center space-x-3 hover:bg-gray-200 transition-all transform hover:scale-105"
              >
                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M8 5v14l11-7z"/>
                </svg>
